...
def _yt_dlp_proxy_args() -> list:
    """Return proxy args for yt-dlp if Webshare credentials are configured."""
    if WEBSHARE_USERNAME and WEBSHARE_PASSWORD:
        return ["--proxy", f"http://{WEBSHARE_USERNAME}:{WEBSHARE_PASSWORD}@p.webshare.io:80"]
    return []
...
async def get_transcript(
    ...
    # ── Step 0: youtube-transcript-api via Webshare proxy ────────────────────
    try:
        proxy_config = WebshareProxyConfig(
            proxy_username=WEBSHARE_USERNAME,
            proxy_password=WEBSHARE_PASSWORD,
        ) if WEBSHARE_USERNAME else None
        ...
        if transcript:
            return {"transcript": transcript, "source": "transcript-api"}
    except (NoTranscriptFound, TranscriptsDisabled):
        pass  # no captions available, fall through
    except Exception:
        pass  # any other error, fall through

    with tempfile.TemporaryDirectory() as tmp_dir:
        # ── Step 1: Try subtitle extraction via yt-dlp ─────────────────────────
        try:
